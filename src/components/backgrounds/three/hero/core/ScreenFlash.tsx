...
export const ScreenFlash = () => {
  ...
  useFrame(() => {
    ...
    if (flashImplosion.progress > 0 && flashHole.progress === 0) {
      if (flashImplosion.progress < 0.25) { // 0.2s ramp up
    ...
    // Flash 2: Light Absorption into Singularity
      ...
      if (flashHole.progress < 0.6) {
        opacity = Math.pow(flashHole.progress / 0.6, 1.5) * 0.72;
        hex = '#000000';
      } else {
    ...
    // The Eerie Silence Gap
       ...
       opacity = 1.0 - Math.pow(flashSilence.progress, 2); 
       hex = '#000000';
    }
    ...
    */
    // Flash 4: Hyperspace Exit Flash
      ...
      if (flashJump.progress < 0.1) {
        ...
        opacity = Math.pow(1 - ((flashJump.progress - 0.1) / 0.9), 2) * 0.8;
      }
