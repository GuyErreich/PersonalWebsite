...
export const useStarsSound = (skipIntro: boolean, orchestrator: AnimationOrchestrator) => {
  ...
  useFrame(() => {
    ...
    try {
      ...
      const baseFreqs = [587.33, 739.99, 880.0, 1174.66, 1479.98, 1760.0];
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const freq =
          baseFreqs[Math.floor(Math.random() * baseFreqs.length)] + (Math.random() * 4 - 2);
