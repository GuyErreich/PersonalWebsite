...
export const ImplosionCore = () => {
  ...
  useFrame(() => {
    ...
    if (activeT < 0.6) {
      ...
      const c1 = 1.70158;
      const c3 = c1 + 1;
      introBounce = Math.max(0, 1.0 + c3 * (x - 1) ** 3 + c1 * (x - 1) ** 2);
    }

    const finalPinch = progress > 0.88 ? Math.max(0, 1 - (progress - 0.88) * (1 / 0.12)) : 1;
    const coreCollapse = 1.0 - progress ** 2.5 * 0.9;
    ...
    horizonRef.current.scale.setScalar(
      (shake * coreCollapse * 1.3 + expandCollapse * 0.5) * finalPinch * introBounce,
    );
    ...
    (horizonRef.current.material as THREE.MeshBasicMaterial).opacity =
      (0.2 + expandCollapse * 0.8) * finalPinch;
  });
