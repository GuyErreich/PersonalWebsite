...
export const ImplosionWormholes = ({ count = 400 }: { count?: number }) => {
    ...
    const WORMHOLE_EFFECT_DURATION = 0.10;
    ...
    const { initialPositions, startProgresses, colors } = useMemo(() => {
        ...
        return { initialPositions: _positions, startProgresses: _startP, colors: _colors };
    }, [count]);
    ...
    useFrame(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            wColor[i * 6]     = 1.0; wColor[i * 6 + 1] = 1.0; wColor[i * 6 + 2] = 1.0;                                                                                
            if (colors[i] === 0) { 
                wColor[i * 6 + 3] = 0.1; wColor[i * 6 + 4] = 0.55; wColor[i * 6 + 5] = 1.0;                                                                           
            } else {              
                wColor[i * 6 + 3] = 0.7; wColor[i * 6 + 4] = 0.1;  wColor[i * 6 + 5] = 0.9;                                                                           
            }

            if (localProgress < 0) {
                // Not started yet. Show as dust.
                bPos[i * 3] = oX; bPos[i * 3 + 1] = oY; bPos[i * 3 + 2] = oZ;
                // Line is collapsed at outer and invisible
                wPos[i * 6] = oX; wPos[i * 6 + 1] = oY; wPos[i * 6 + 2] = oZ;
                wPos[i * 6 + 3] = oX; wPos[i * 6 + 4] = oY; wPos[i * 6 + 5] = oZ;
                wAlpha[i * 2] = 0; wAlpha[i * 2 + 1] = 0;
            } else if (localProgress >= 1.0) {
                // Finished completely! Hide dust, hide line.
                bPos[i * 3] = 10000; bPos[i * 3 + 1] = 0; bPos[i * 3 + 2] = 0;
                wPos[i * 6] = 0; wPos[i * 6 + 1] = 0; wPos[i * 6 + 2] = 0;
                wPos[i * 6 + 3] = 0; wPos[i * 6 + 4] = 0; wPos[i * 6 + 5] = 0;
                wAlpha[i * 2] = 0; wAlpha[i * 2 + 1] = 0;
            } else {
                ...
                if (localProgress < 0.4) {
                    ...
                    const ease = 1.0 - Math.pow(1.0 - l, 3);
                    wPos[i * 6 + 3] = oX * (1 - ease);
                    wPos[i * 6 + 4] = oY * (1 - ease);
                    wPos[i * 6 + 5] = oZ * (1 - ease);
                    wAlpha[i * 2] = 1.0; wAlpha[i * 2 + 1] = 0.7;
    ...
    return (
        <group>
            <points ref={blueDustRef}>
                <bufferGeometry>
                    <bufferAttribute attach="attributes-position" args={[blueDustPositions, 3]} count={count} array={blueDustPositions} itemSize={3} />                                                                                  
                </bufferGeometry>
                <shaderMaterial
            ...
            <lineSegments ref={wormholeRef}>
                <bufferGeometry>
                    <bufferAttribute attach="attributes-position" args={[wormholePositions, 3]} count={count * 2} array={wormholePositions} itemSize={3} />                                                                                      
                    <bufferAttribute attach="attributes-aAlpha"   args={[wormholeAlphas,    1]} count={count * 2} array={wormholeAlphas}    itemSize={1} />                                                                                    
                    <bufferAttribute attach="attributes-aColor"   args={[wormholeColors,    3]} count={count * 2} array={wormholeColors}    itemSize={3} />                                                                                
                </bufferGeometry>
                <shaderMaterial
