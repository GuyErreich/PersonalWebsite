...
const GalleryGridItem = ({ item, index, isVideo }: { item: GameDevItem; index: number; isVideo: (url: string) => boolean }) => {
  ...
  return (
    ...
    >
      {isVideo(item.media_url) ? (
        <video src={item.media_url} poster={item.thumbnail_url} muted loop autoPlay playsInline
...
const GalleryDetailItem = ({ item, index, iconMap }: { item: GameDevItem; index: number; iconMap: Record<string, React.ElementType> }) => {
  const isRevealed = useContext(SectionRevealContext);
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true });
