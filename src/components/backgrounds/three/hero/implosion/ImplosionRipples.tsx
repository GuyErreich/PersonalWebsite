    ...
    rippleRefs.forEach((ripple) => {
      ...
      if (progress > 0.82) {
        ...
        if (currentLoop > lockLoop) {
          ...
          cycle = cycle + (1.0 - cycle) * suckPhase ** 1.5;
        }
      }

      const currentScale = Math.max(0, 1.0 - cycle ** 0.5) * ripple.scaleMultiplier;
      const warp = cycle ** 2.0 * 0.2;
      ...
      if (mat) {
        const sceneClimaxPulse = progress ** 3.0;
        const intensity = 1.0 + cycle ** 1.5 * 3.0 + sceneClimaxPulse * 15.0;

        const startFade = cycle === 1.0 ? 0 : Math.min(1.0, cycle * 5.0);
        const endFade = Math.max(0, 1.0 - cycle ** 3.0);
