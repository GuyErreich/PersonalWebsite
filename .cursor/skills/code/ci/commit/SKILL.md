...
1. **Confirm consent.** Only commit when the user explicitly asked to commit (see `.cursor/rules/behaviors/git-commit-consent.mdc`).
...
7. **Do not push.** Pushing requires separate explicit consent (`.cursor/rules/behaviors/git-push-consent.mdc`).
